...
contains
    ...
    pure subroutine calcTaxCoef(tax)
        implicit none
        type(bcout3d_t), intent(inout) :: tax
        integer :: iX1, iX2
        real(dp) :: P(3), Q(3), R(3)
        real(dp) :: u(3)
        real(dp) :: v(3)
        ...
        ! differenced breaks
        do iX1 = 1, tax%nearn1-1
            tax%dearnbr1(iX1) = ( tax%earnbr1(iX1+1) - tax%earnbr1(iX1) )
        end do

        do iX2 = 1, tax%nearn2-1
            tax%dearnbr2(iX2) = ( tax%earnbr2(iX2+1) - tax%earnbr1(iX2) )
        end do
        ...
        do iX2 = 1, tax%nearn2-1
            do iX1 = 1, tax%nearn1-1
                ...
                Q(1) = tax%earnbr1(iX1)
                Q(2) = tax%earnbr2(iX2+1)
                Q(3) = tax%tCou(iX1,iX2+1)
                R(1) = tax%earnbr1(iX1+1)
                R(2) = tax%earnbr2(iX2)
                ...
                ! upper triangle
                P(1) = tax%earnbr1(iX1+1)
                P(2) = tax%earnbr2(iX2)
                P(3) = tax%tCou(iX1+1,iX2)
                Q(1) = tax%earnbr1(iX1+1)
                Q(2) = tax%earnbr2(iX2+1)
                Q(3) = tax%tCou(iX1+1,iX2+1)
                R(1) = tax%earnbr1(iX1)
                R(2) = tax%earnbr2(iX2+1)
    ...
    pure function evalTaxCouScalar(tax,earn1,earn2) result(y)
        ...
        if (earn1<=tax%earnbr1(1) .and. earn2<=tax%earnbr2(1)) then
            y = tax%tCouL(1,1,1)
        else if (earn1>=tax%earnbr1(tax%nearn1) .and. earn2>=tax%earnbr2(tax%nearn2)) then
            y = tax%tCouU(1,tax%nearn1-1,tax%nearn2-1) + tax%tCouU(2,tax%nearn1-1,tax%nearn2-1)*earn1 &
              + tax%tCouU(3,tax%nearn1-1,tax%nearn2-1)*earn2
        else
            do iX2 = 1, tax%nearn2-1
                if ( earn2>=tax%earnbr2(iX2) .and. ( earn2<tax%earnbr2(iX2+1) .or. (iX2==tax%nearn2-1 .and. earn2>=tax%earnbr2(tax%nearn2)) ) ) then
                    do iX1 = 1, tax%nearn1-1
                        if ( earn1>=tax%earnbr1(iX1) .and. ( earn1<tax%earnbr1(iX1+1) .or. (iX1==tax%nearn1-1 .and. earn1>=tax%earnbr1(tax%nearn1) ) ) ) then
                            if ( earn1-tax%earnbr1(iX1) + (earn2-tax%earnbr2(iX2))*tax%dearnbr1(iX1)/tax%dearnbr2(iX2) >=(tax%dearnbr1(iX1)) ) then
...
!     end subroutine setTax
    ...
    subroutine printTaxX(tax)
        ...
        do ixL1 = 1, tax%nearn2-1
            do ixL0 = 1, tax%nearn1-1
                ixParam = ixParam+1
                print *, ixParam, tax%tCou(ixL0,ixL1), (tax%tCou(ixL0,ixL1)-tax%tCou(ixL0-1,ixL1))/tax%dearnbr1(ixL0-1)
            end do
            ixParam = ixParam+1
            print *, ixParam, tax%tCou(ixL1+1,ixL1+1), (tax%tCou(ixL1+1,ixL1+1)-tax%tCou(ixL1+1,ixL1))/tax%dearnbr2(ixL1)
        end do
...
!     end subroutine kinkshours
    ...
    subroutine kinksearn3d(sys,fam,hours1,hours2,earn1,earn2,bcout,taxlevel,taxout,verbose)
        ...
        type(fam_t)                :: fam0
        type(net_t), target        :: net
        real(dp)                   :: taxcomp0, taxcomp1, taxcomp2
        real(dp)                   :: taxrate0, taxrate1, taxrate2
        real(dp)                   :: earn, earn0 !,ern0,ernb !,ern0b
        integer                    :: i !,step, stepb, i

        integer :: iX1, iX2
        logical, parameter :: domtr = .false. ! for future use
...
!         kinks_hrs2     = hours2
        ...
        bcout%earnbr1(1:nearn1) = earn1
        bcout%earnbr2(1:nearn2) = earn2
        bcout%nearn1 = nearn1
        bcout%nearn2 = nearn2
        ...
        do iX2 = 1, nearn2
            do iX1 = 1, nearn1
                ...
                bcout%tCou(iX1,iX2) = taxcomp0
                bcout%mtr1(iX1,iX2) = taxrate1
                bcout%mtr2(iX1,iX2) = taxrate2
