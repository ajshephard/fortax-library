...
!                    & + net%tu%chben+net%tu%ctc-appamt, 0.0_dp)*sys%ccben%taper,0.0_dp)
    ...
    real(dp) pure function HBAppAmt(sys,fam)
        ...
        if (.not. _famcouple_) then
            if (_famkids_) then
                !Lone parent
                !sys%rebatesys%PremLP abolished Apr 98, but don't need to condition on year here because parameter
                !should be zero for later years
                if (fam%ad(1)%age < 18) then
        ...
        if (_famkids_) then
            ...
            do i = 1, fam%nkids
                do j = 1, sys%rebatesys%NumAgeRng
                    if ((fam%kidage(i) >= sys%rebatesys%AgeRngl(j)) &
                        .and. (fam%kidage(i) <= sys%rebatesys%AgeRngu(j))) then
